...
function TargetingMannequin({ type, selectedLimb, onSelectLimb, accentColor }: MannequinProps) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", zIndex: 5, position: "relative", background: "rgba(5, 5, 5, 0.85)", padding: "10px", border: "1px solid rgba(255, 255, 255, 0.05)" }}>
      <span style={{ fontSize: "11px", color: accentColor, fontWeight: "bold", letterSpacing: "0.1em", textTransform: "uppercase" }}>
        {type === "ATTACK" ? "STRIKE TARGET" : "SHIELD SECTOR"}
      </span>
      <div style={{ position: "relative", width: "90px", height: "120px" }}>
        <svg width="80" height="110" viewBox="0 0 80 120" style={{ display: "block", margin: "auto" }}>
          ...
          <circle
            ...
            cy="18"
            r="10"
            fill={selectedLimb === "HEAD" ? accentColor : "rgba(10, 10, 10, 0.8)"}
            stroke={accentColor}
            strokeWidth="2"
          ...
          <rect
            ...
            height="38"
            rx="2"
            fill={selectedLimb === "TORSO" ? accentColor : "rgba(10, 10, 10, 0.8)"}
            stroke={accentColor}
            strokeWidth="2"
            style={{ cursor: "pointer", transition: "all 0.15s" }}
          ...
          <rect
            ...
            height="32"
            rx="1"
            fill={selectedLimb === "ARMS" ? accentColor : "rgba(10, 10, 10, 0.8)"}
            stroke={accentColor}
            strokeWidth="2"
            style={{ cursor: "pointer", transition: "all 0.15s" }}
          ...
          <rect
            ...
            height="40"
            rx="1"
            fill={selectedLimb === "LEGS" ? accentColor : "rgba(10, 10, 10, 0.8)"}
            stroke={accentColor}
            strokeWidth="2"
            style={{ cursor: "pointer", transition: "all 0.15s" }}
      ...
      <span style={{ fontSize: "12px", fontFamily: "Orbitron, sans-serif", color: "#ffffff", fontWeight: "900" }}>
...
export default function ArenaPage() {
  ...
  const [profileName, setProfileName] = useState<string>("REDQUEEN_X");
  ...
  const handleSendChat = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const replies = [
        ...
        "USDC POOL IS MINE.",
        "CALCULATION ERROR ON YOUR DEFENSIVE SHIELD.",
        "CHECK YOUR LIMB INTEGRITY.",
        "NICE SETPIN SYSTEM, BUT MINE IS OUTPERFORMING."
      ];
  ...
  const submitTacticalMoves = () => {
    ...
    if (opponentDefense === selectedAttack) {
      ...
      newLogs.push({ time: timeStr, sender: "SYSTEM", text: `DEFLECTED! OPPONENT BLOCKED STRIKE ON THEIR [${selectedAttack}].`, color: "#ffc107" });
    }
    ...
    if (hitRoll <= playerHitChance) {
      ...
      if (isCrit) {
        ...
        newLogs.push({ time: timeStr, sender: profileName, text: `💥 CRITICAL HIT! DEALT ${dmg} DAMAGE TO OPPONENT'S [${selectedAttack}].`, color: "#00ffcc" });
      } else {
        newLogs.push({ time: timeStr, sender: profileName, text: `DIRECT STRIKE! DEALT ${dmg} DAMAGE TO OPPONENT'S [${selectedAttack}].` });
      }
    ...
    if (oppHitRoll <= oppHitChance) {
      ...
      if (oppIsCrit) {
        ...
        newLogs.push({ time: timeStr, sender: "DESGECEAN", text: `💥 CRITICAL STRIKE! YOUR [${opponentAttack}] TOOK ${oppDmg} DAMAGE.`, color: "#ff003c" });
      } else {
        newLogs.push({ time: timeStr, sender: "DESGECEAN", text: `HOSTILE HIT! YOUR [${opponentAttack}] TOOK ${oppDmg} DAMAGE.` });
      }
    ...
    if (oppHp <= 0 && plyHp <= 0) {
      ...
      setMatchActive(false);
      newLogs.push({ time: timeStr, sender: "SYSTEM", text: "🏆 VICTORY! OPPONENT ELIMINATED. +10 $THREAT SECURED.", color: "#00ffcc" });
    } else if (plyHp <= 0) {
      setCombatOutcome("lose");
      setMatchActive(false);
      newLogs.push({ time: timeStr, sender: "SYSTEM", text: "❌ DEFEAT: SYSTEMS OFFLINE. LOST 10 $THREAT.", color: "#ff003c" });
    }
  ...
  return (
    <div id="game-arena-root" className={shakeTrigger ? "hud-screen-shake" : ""} style={{ background: "#000000", minHeight: "100vh", height: "100vh", color: "#ffffff", fontFamily: "Rajdhani, sans-serif", padding: "20px 24px", position: "relative", boxSizing: "border-box", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
      {/* HUD scanline and dynamic sweeps */}
      ...
      {/* Main 1v1 Symmetrical Arena HUD Grid */}
      <main style={{ display: "grid", gridTemplateColumns: "1.15fr 1fr 1.15fr", gap: "20px", flexGrow: 1, alignItems: "center", position: "relative", zIndex: 10, minHeight: "0", margin: "12px 0" }}>
        ...
        <div style={{ display: "flex", gap: "20px", alignItems: "center", height: "100%", position: "relative", padding: "12px 0" }}>
          ...
          <img
            ...
            style={{
              ...
              width: "auto",
              opacity: 0.18,
              filter: "drop-shadow(0 0 30px rgba(255, 0, 60, 0.7)) brightness(0.6)",
              zIndex: 1,
              pointerEvents: "none"
            }}
          />
          ...
          <div style={{ flexGrow: 1, display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%", maxWidth: "260px", zIndex: 3, background: "rgba(5, 5, 5, 0.78)", padding: "14px", border: "1px solid rgba(255,0,60,0.1)", borderRadius: "2px", backdropFilter: "blur(4px)" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", color: "#ff003c" }}>
                <span style={{ fontSize: "20px", fontWeight: "900", fontFamily: "Orbitron, sans-serif", letterSpacing: "0.08em", textShadow: "0 0 8px rgba(255, 0, 60, 0.6)" }}>
                  {profileName}
                </span>
                <span style={{ fontSize: "14px" }}>★</span>
              </div>
              
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "#8a8a8a", marginTop: "4px", fontFamily: "Rajdhani, sans-serif", letterSpacing: "0.15em", fontWeight: 700 }}>
            ...
            {/* Defensive body sector select mannequin */}
        ...
        <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", alignItems: "center", padding: "10px 0", height: "100%", zIndex: 5, background: "rgba(5, 5, 5, 0.45)" }}>
          ...
          <div style={{ textAlign: "center", width: "100%", display: "flex", flexDirection: "column", gap: "8px", fontFamily: "Rajdhani, sans-serif" }}>
            <div>
              ...
              <div style={{ fontSize: "14px", fontWeight: "bold", color: "#ffffff", letterSpacing: "0.08em" }}>WASTELAND COLISEUM</div>
            </div>
            
            <div>
              <span style={{ fontSize: "11px", color: "#8a8a8a", letterSpacing: "0.15em", fontWeight: 700 }}>ESCROW RISK WAGER</span>
              <div style={{ fontSize: "16px", fontWeight: "bold", color: "#ff003c", textShadow: "0 0 8px rgba(255, 0, 60, 0.5)" }}>10 $THREAT</div>
            </div>
          </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                <div style={{ display: "flex", gap: "6px" }}>
                  <div style={{ flex: 1, border: "1px solid rgba(255, 0, 60, 0.2)", background: "rgba(5, 5, 5, 0.9)", padding: "6px", fontSize: "11px", textAlign: "center", fontFamily: "Rajdhani, sans-serif", fontWeight: 700, color: "#ff003c" }}>
                    ATTACK STRIKE: {selectedAttack || "NOT SET"}
                  </div>
                  <div style={{ flex: 1, border: "1px solid rgba(0, 255, 204, 0.2)", background: "rgba(5, 5, 5, 0.9)", padding: "6px", fontSize: "11px", textAlign: "center", fontFamily: "Rajdhani, sans-serif", fontWeight: 700, color: "#00ffcc" }}>
                    SHIELD BARRIER: {selectedDefense || "NOT SET"}
                  </div>
                </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: "12px",
                    padding: "11px",
                    opacity: (!selectedAttack || !selectedDefense || !!combatOutcome) ? 0.35 : 1,
                    cursor: (!selectedAttack || !selectedDefense || !!combatOutcome) ? "not-allowed" : "pointer"
                  }}
                >
        ...
        {/* PLAYER B (Right Panel - Soldier/Operative) */}
        <div style={{ display: "flex", gap: "20px", alignItems: "center", height: "100%", justifyContent: "flex-end", position: "relative", padding: "12px 0" }}>
          ...
          <img
            ...
            style={{
              ...
              width: "auto",
              opacity: 0.18,
              filter: "drop-shadow(0 0 30px rgba(255, 255, 255, 0.3)) brightness(0.7)",
              zIndex: 1,
              pointerEvents: "none"
            }}
          />
          ...
          {/* Player stats overlaid */}
          <div style={{ flexGrow: 1, display: "flex", flexDirection: "column", justifyContent: "space-between", height: "100%", maxWidth: "260px", zIndex: 3, background: "rgba(5, 5, 5, 0.78)", padding: "14px", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "2px", backdropFilter: "blur(4px)" }}>
            <div style={{ textAlign: "right" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "8px" }}>
                <span style={{ fontSize: "20px", fontWeight: "900", fontFamily: "Orbitron, sans-serif", letterSpacing: "0.08em", color: "#ffffff", textShadow: "0 0 8px rgba(255, 255, 255, 0.2)" }}>
                  DESGECEAN
                </span>
            ...
            {/* Attack targeting selector mannequin */}
            <div style={{ marginTop: "14px", display: "flex", justifyContent: "center" }}>
              <TargetingMannequin
                type="ATTACK"
      ...
      <footer style={{ display: "grid", gridTemplateColumns: "1.15fr 1fr 1.15fr", gap: "16px", minHeight: "190px", maxHeight: "210px", position: "relative", zIndex: 20 }}>
        ...
        <div className="hud-panel" style={{ padding: "12px", display: "flex", flexDirection: "column", gap: "6px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px", flexGrow: 1, minHeight: "0" }}>
            {/* Mutator 1: TOXIC FOG */}
            <div style={{ border: "1px solid rgba(255, 0, 60, 0.12)", background: "rgba(5, 5, 5, 0.6)", padding: "6px", display: "flex", flexDirection: "column", justifyContent: "space-between", borderRadius: "1px" }}>
